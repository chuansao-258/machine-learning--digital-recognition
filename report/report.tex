...
\begin{document}
	...
	\subsection{NB}
	 ...
	 The naive bayes model from sklearn does not work well, with only 58\% accuracy, so we try to implement one by ourselves. For training data, we calculate two thing: the probabilities of an image being labeled as each class; and the probabilities of the gray value of a pixel being $0$ in each class, which means we choose every pixel as a feature. \\ \\
